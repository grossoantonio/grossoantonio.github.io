$(document).ready(function () {
  ...
  $.each(cat.list, function (index, value) {
    ...
    if (index === 0) {
      ...
      $(".controls-wrap .select-cat ul").append(
        '<li class="selected" data-cat="' + catName + '"><span>' + catName + "</span></li>"
      );
      $.each(this["cod-list"], function (i, code) {
        var v = code.replace(/\-/g, " ");
        if (i === 0) {
          $(".controls-wrap .select-cyl ul").append(
            '<li class="selected" data-cod="' +
              code +
              '" data-cat-en="' +
              catNameEn +
              '"><span>' +
          ...
          $(".elements-wrap .element-img").append(
            '<div class="img-cilindro" style="background-image:url(/assets/img/cilindri/' +
              code +
              '.jpg)"></div>'
          );
        } else {
      ...
      $(".controls-wrap .select-cat ul").append(
        '<li data-cat="' + catName + '"><span>' + catName + "</span></li>"
      );
    }
  });
  ...
  $(".controls-wrap .select-cat ul li").click(function () {
    ...
    $.each(cat.list, function (index, value) {
      ...
      if (value.name_it == data || value.name_en == data) {
        console.log(data);
        ...
        $.each(this["cod-list"], function (i, code) {
          var v = code.replace(/\-/g, " ");
          var liData =
            '<li data-cod="' + code + '" style="display:none;"><span>' + v + "</span></li>";
          if (i === 0) {
            $(
              '<li class="selected" data-cod="' +
                code +
              ...
              .appendTo(".controls-wrap .select-cyl ul")
              .fadeIn(300 * i + 1);
            $(".elements-wrap .element-img .img-cilindro").fadeOut(500, function () {
              $(this).remove();
            });
            $(".elements-wrap .element-img").append(
              '<div class="img-cilindro" style="background-image:url(/assets/img/cilindri/' +
                code +
                '.jpg); display:none;"></div>'
            );
